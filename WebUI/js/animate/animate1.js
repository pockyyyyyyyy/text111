...
$(function () {
    ...
    GNB();
    //第一屏打字效果调用
    sayHello($(".hellotext_wrapper"), '欢迎来到，带刺の电鸡   快 乐 安 全 出 行');
    ...
    $(".LXFS").click(function () {
        $("#gudong").fadeIn(500);
        $("#gudong .pop_content").animate({ right: (popW - 2800) / 2 }, 500);
    });
    ...
    $(".CDZR").click(function () {
        $("#chedui").fadeIn(500);
        $("#chedui .pop_content").animate({ right: (popW - 2800) / 2 }, 500);
    });
    $("#cheduiclose").click(function () {
        $("#chedui").fadeOut(500);
        $("#chedui .pop_content").animate({ right: -(popW - 2800) / 2 }, 500);
    });
...
//弹出层禁掉滚轮事件2
$("#chedui").bind('mousewheel', function (event, delta) {
    event.stopPropagation();
    return false;
});